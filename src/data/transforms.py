...
def get_train_transforms(img_height: int = 32, img_width: int = 128) -> A.Compose:
    ...
    return A.Compose([
        ...
        A.Affine(
            ...
            rotate=(-10, 10),  # slightly increased rotation
            shear=(-5, 5),     # added shear for extra variation
            fill=128,
            p=0.5
        ),
        A.Perspective(scale=(0.02, 0.05), p=0.3),
        A.RandomBrightnessContrast(
            brightness_limit=0.3,  # increased limits
            contrast_limit=0.3,
            p=0.7  # slightly higher probability
        ),
        ...
        A.ImageCompression(quality_range=(25, 70), p=0.35),  # compression diversity
        A.GaussNoise(std_range=(0.01, 0.04), p=0.25),        # mild ISO/sensor noise
        A.GaussianBlur(blur_limit=(3, 7), p=0.25),           # increased blur limit range
